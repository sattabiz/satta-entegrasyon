...
echo ======================================================================
echo   SATTA ENTEGRASYON - LEGACY SURUM PAKETLEME (.venv_legacy - PySide2)
echo ======================================================================
echo.
...
:: 3. PyInstaller Derlemesi (.venv_legacy)
echo.
echo [2/3] PyInstaller ile SattaEntegrasyon_Legacy derleniyor (.venv_legacy)...
set "PYINSTALLER_CMD="
if exist ".venv_legacy\Scripts\pyinstaller.exe" set "PYINSTALLER_CMD=.venv_legacy\Scripts\pyinstaller.exe"
if not defined PYINSTALLER_CMD set "PYINSTALLER_CMD=pyinstaller"

echo Calistiriliyor: %PYINSTALLER_CMD% --noconfirm --clean SattaEntegrasyon_Legacy.spec
call %PYINSTALLER_CMD% --noconfirm --clean SattaEntegrasyon_Legacy.spec
if %ERRORLEVEL% NEQ 0 goto PY_ERROR

if not exist "dist\SattaEntegrasyon_Legacy\SattaEntegrasyon_Legacy.exe" goto PY_MISSING
echo [OK] PyInstaller Legacy derlemesi tamamlandi.
goto RUN_INNO

:PY_ERROR
echo [HATA] PyInstaller Legacy derlemesi basarisiz oldu.
pause
exit /b %ERRORLEVEL%

:PY_MISSING
echo [HATA] dist\SattaEntegrasyon_Legacy\SattaEntegrasyon_Legacy.exe olusturulamadi.
pause
exit /b 1
...
:RUN_INNO
echo.
echo [3/3] Inno Setup ile kurulum paketi olusturuluyor (setup_legacy.iss, v%APP_VERSION%)...
"%ISCC%" /DMyAppVersion="%APP_VERSION%" "installer\setup_legacy.iss"
if %ERRORLEVEL% NEQ 0 goto INNO_ERROR

echo.
echo ======================================================================
echo   [BASARILI] Legacy Kurulum Paketi Basariyla Olusturuldu.
echo   Versiyon     : %APP_VERSION%
echo   Cikti Dosyasi: installer\Output\SattaEntegrasyon-Setup-Legacy.exe
echo ======================================================================
echo.
pause
exit /b 0
