...
class SattaServicePushConnector:
    ...
    def push_service(self, service_data: Dict[str, Any]) -> Dict[str, Any]:
        payload = self._build_payload([service_data])
        return self._post_services(payload)

    def push_services(self, services: Iterable[Dict[str, Any]]) -> Dict[str, Any]:
        service_list = [service for service in services if isinstance(service, dict)]
        if not service_list:
            raise ValueError("Gönderilecek hizmet listesi boş olamaz.")

        payload = self._build_payload(service_list)
        return self._post_services(payload)
    ...
    def _post_services(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        if not self.token:
        ...
        try:
            ...
            raise RuntimeError(f"Satta hizmet gönderme isteği başarısız oldu: {exc}") from exc
        ...
        if not response.ok:
            ...
            raise RuntimeError(
                f"Satta hizmetleri oluşturamadı. HTTP {response.status_code}. {message}"
            )
